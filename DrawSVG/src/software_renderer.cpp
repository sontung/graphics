...
void SoftwareRendererImp::set_sample_rate( size_t sample_rate ) {
    ...
    memset(&sample_buffer[0], 0, 4 * sample_buffer_w * sample_buffer_h);
...
void SoftwareRendererImp::rasterize_triangle( float x0, float y0,
    ...
    float sample_step = 1.0/(float)sample_rate;
...
void SoftwareRendererImp::fill_sample( int x, int y, const Color& color ) {
    // fill in the nearest pixel
    int sx = static_cast<int> (x*sample_rate-0.5);
    int sy = static_cast<int> (y*sample_rate-0.5);
...
void SoftwareRendererImp::fill_pixel(int x, int y) {
    ...
    while (x_start<x_end) {
        ...
        while (y_start<y_end) {
            sx = static_cast<int> (x_start*sample_rate-0.5);
            sy = static_cast<int> (y_start*sample_rate-0.5);
    ...
    if (counter > 0) {
        ...
        render_target[4 * (x + y * target_w)    ] = (uint8_t) (r_all/counter*255);
        render_target[4 * (x + y * target_w) + 1] = (uint8_t) (g_all/counter*255);
        render_target[4 * (x + y * target_w) + 2] = (uint8_t) (b_all/counter*255);
        render_target[4 * (x + y * target_w) + 3] = (uint8_t) (a_all/counter*255);
    }
}
